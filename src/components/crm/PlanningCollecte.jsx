...
export default function PlanningCollecte({ clients, interventions, onRefresh }) {
  ...
  const clientsJour = clients.filter(c =>
    c.jours_collecte && c.jours_collecte.includes(selectedJour)
  );

  const dejaPasses = new Set(
    interventions
      .filter(i => i.date === today && i.type_intervention === "Ramassage des ordures")
      .map(i => i.client_id)
  );
  ...
  async function validerPassages() {
    ...
    await Promise.all(aValider.map(c =>
      base44.entities.Intervention.create({
        client_id: c.id,
        nom_client: c.nom,
        type_intervention: "Ramassage des ordures",
        date: today,
        jour_semaine: selectedJour,
        statut: "Réalisée",
        description: `Collecte du ${today} — ${selectedJour}`,
      })
    ));
    setCoches({});
    setSaving(false);
  ...
  return (
    <div>
      <div className="grid grid-cols-7 gap-1 mb-5">
        {JOURS.map(jour => {
          const count = clients.filter(c => c.jours_collecte?.includes(jour)).length;
          ...
          return (
            ...
            >
              ...
              <span className="text-[8px] opacity-70">ménage{count > 1 ? "s" : ""}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{clientsJour.length} ménage(s) à collecter</p>
        </div>
        {nbCoches > 0 && (
      ...
      {clientsJour.length === 0 ? (
        <div className="text-center py-10 text-muted-foreground">
          ...
          <p className="text-sm">Aucun ménage planifié ce jour.</p>
          <p className="text-xs mt-1">Ajoutez des jours de collecte dans la fiche client.</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    {client.adresse} · {client.nombre_poubelles || 1} poubelle(s)
                  </p>
                </div>
                {dejaPasse && (
